...
/**
 * @title TwitchCardVault
 * @notice Vault contract for temporarily storing gift cards sent to Twitch usernames
 * 
 * Flow:
 * 1. User creates gift card via createGiftCardForTwitch (in GiftCard contract)
 * 2. Card is minted directly to Vault and registered with username
 * 3. Claimer verifies ownership of Twitch username off-chain and calls claimCard
 * 4. Vault transfers the NFT to claimer's wallet
 */
contract TwitchCardVault is Ownable, IERC721Receiver {
    ...
    /**
     * @notice Deposit a gift card for a Twitch username
    ...
    /**
     * @notice Claim a gift card by the owner of the Twitch username
     * @dev Claimer must prove ownership of username off-chain before calling
     * @param tokenId The token ID to claim
     * @param username The Twitch username (must match token)
